...
func main() {
	...
	fmt.Println(time.Now().Format("15:04:05"), "创建普通订单:", o1.OrderNo)
	o2 := service.CreateOrder(2, 100, 88.8, true)
	fmt.Println(time.Now().Format("15:04:05"), "创建VIP订单:", o2.OrderNo)

	// 启动机器人
	rid := service.AddRobot()
	fmt.Println(time.Now().Format("15:04:05"), "启动机器人:", rid)

	// 等待订单完成
	time.Sleep(12 * time.Second)
	orders := service.GetFinishedOrders()
	for _, o := range orders {
		fmt.Printf("%s 订单完成: %s 用户:%d 金额:%.2f\n", time.Now().Format("15:04:05"), o.OrderNo, o.UserID, o.Amount)
	}
